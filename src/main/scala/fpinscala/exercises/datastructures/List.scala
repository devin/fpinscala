...
object List: // `List` companion object. Contains functions for creating and working with lists.
  ...
  def tail[A](l: List[A]): List[A] = l match
    case Nil => sys.error("cannot tail an empty list.")
    case Cons(_, t) => t
  ...
  def init[A](l: List[A]): List[A] = l match
    case Nil => sys.error("cannot init on an empty list.")
    case Cons(_, Nil) => Nil
  ...
  def appendViaFoldRight[A](l: List[A], r: List[A]): List[A] = foldRight(
    l,
    r,
    (a, acc) => Cons(a, acc)
  )

  def concat[A](l: List[List[A]]): List[A] = foldRight(
    l,
    Nil: List[A],
    (a, acc) => appendViaFoldRight(a, acc)
  )
